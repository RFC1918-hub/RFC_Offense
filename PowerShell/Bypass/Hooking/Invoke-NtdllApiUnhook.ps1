<#
.SYNOPSIS
    Unhook a function from ntdll.dll
.DESCRIPTION
    Unhook a function from ntdll.dll
...
.EXAMPLE
    .\Invoke-NtdllApiUnhook.ps1 -FunctionName "NtCreateFile"
.EXAMPLE
    .\Invoke-NtdllApiUnhook.ps1 -All
.EXAMPLE
    .\Invoke-NtdllApiUnhook.ps1 -List
#>
...
function GetNeighborPatch([IntPtr]$FunctionAddress) {
    $patch = [byte[]]@(0x4C, 0x8B, 0xD1, 0xB8, 0xFF)
    $count = 0

    while ($true) {
        $count++
        $NeighborBaseAddress = [System.IntPtr]($FunctionAddress.ToInt64() + 32)
        $NeighborFunctionInstruction = [byte[]]::new(5)
        [System.Runtime.InteropServices.Marshal]::Copy($NeighborBaseAddress, $NeighborFunctionInstruction, 0, 5)

        Write-Host "`tNeighborBaseAddress: $($NeighborBaseAddress.ToString("X"))"
        Write-Host "`tNeighborFunctionInstruction: $(([System.BitConverter]::ToString($NeighborFunctionInstruction)))"

        if (IsHooked($NeighborFunctionInstruction)) {
            Write-Host "`tNeighborFunctionInstruction is hooked. Trying next neighbor." -ForegroundColor Yellow
            $FunctionAddress = $NeighborBaseAddress
        } else {
            Write-Host "`tNeighborFunctionInstruction is not hooked."
            $patch[4] = $NeighborFunctionInstruction[4] - $count
            Write-Host "`tPatch: $(([System.BitConverter]::ToString($patch)))"
            return $patch
        }
    }
}

function GetNtdllApiHooks() {
    ...
    for ($i = 0; $i -lt $ExportDirectory.NumberOfNames - 1; $i++) {
        ...
        if ($ExportFunctionName -like "nt*" -or $ExportFunctionName -like "zw*") {
            ...
            if (IsHooked($FunctionInstruction)) {
                Write-Host "[-] "$ExportFunctionName "is hooked (" $ExportFunctionAddress.ToString("X") ") - (" ([System.BitConverter]::ToString($FunctionInstruction)) ")"  -ForegroundColor Red
                $ExportedFunctions.Add($ExportFunctionName, $ExportFunctionAddress)
            }
        }
    }   
    Write-Host "`n[+] Found $($ExportedFunctions.Count) hooked functions in NtDll.dll`n" -ForegroundColor Green
    return $ExportedFunctions
}

function PatchNtdllApiHooks([System.IntPtr]$FunctionAddress) {
    Write-Host "`n[+] Patching function at address: $($FunctionAddress.ToString("X"))`n" -ForegroundColor Green
    $Patch = GetNeighborPatch($FunctionAddress)
    ...
    Write-Host "`n`t[*] Changing memory protection to PAGE_EXECUTE_READWRITE" -ForegroundColor Green
    $oldProtection = [UInt32]0
    $vp.Invoke($FunctionAddress, [UIntPtr]::new(5), 0x40, [ref]$oldProtection) | Out-Null
    if ($oldProtection -eq 0) {
        Write-Host "`t[-] Failed to change memory protection for ($($FunctionAddress.ToString("X"))) to PAGE_EXECUTE_READWRITE`n" -ForegroundColor Red
        return
    }
    ...
    Write-Host "`t[*] Restoring memory protection`n" -ForegroundColor Green
    $vp.Invoke($FunctionAddress, [UIntPtr]::new(5), $oldProtection, [ref]$oldProtection) | Out-Null
    if ($oldProtection -eq 0) {
        Write-Host "`t[-] Failed to restore memory protection for $($FunctionAddress.ToString("X"))`n" -ForegroundColor Red
        return
    }   
}
...
#--------[Execution]--------#
if ([IntPtr]::Size -eq 4) {
    Write-Host "[-] 32-bit processes are not supported" -ForegroundColor Red
    return
}

if ($List) {
    $null = GetNtdllApiHooks
    return
} elseif ($FunctionName.Length -gt 0) {
    $HookedApiFunctions = GetNtdllApiHooks
    $HookedApiFunction = $HookedApiFunctions[$FunctionName]
    if ($HookedApiFunction) {
        PatchNtdllApiHooks $HookedApiFunction
    } else {
        Write-Host "[-] Function $FunctionName not found`n" -ForegroundColor Red
        return
    }
    Write-Host "`n[+] Hooked function has been patched`n" -ForegroundColor Green
    return
} elseif ($All) {
    $HookedApiFunctions = GetNtdllApiHooks
    foreach ($HookedApiFunction in $HookedApiFunctions.GetEnumerator()) {
        PatchNtdllApiHooks $HookedApiFunction.Value
    }
    Write-Host "`n[+] All hooked functions have been patched`n" -ForegroundColor Green
    return
} else {
    $null = GetNtdllApiHooks
    return
